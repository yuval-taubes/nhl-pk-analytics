"""
Validate trained xG model against expected benchmarks.
Run after xg_model.py completes training.
"""
...
def validate_xg(db, model=None):
    ...
    # Check 2: Rebound xG > non-rebound xG
    ...
    if not rebound_check.empty:
        ...
        if rebound_avg > non_rebound_avg:
            ...
            all_passed = False
        checks['rebound'] = {'rebound_xg': float(rebound_avg), 'non_rebound_xg': float(non_rebound_avg)}
    
    # Check 3: Distance is top negative feature (closer = higher xG)
    check_dist_feature = db.query_to_df("""
        SELECT 
            CORR(CAST(SQRT(POWER(x_norm - 100, 2) + POWER(y_norm - 42.5, 2)) AS numeric), xg) as dist_corr
        FROM shots
        WHERE xg IS NOT NULL AND xg > 0
    """)
    
    if not check_dist_feature.empty:
        corr = check_dist_feature['dist_corr'].iloc[0]
    ...
    # Check 4: xG distribution sanity
    ...
    if not xg_dist.empty:
        ...
        logger.info(f"xG distribution: min={d['min_xg']:.4f}, median={d['median_xg']:.4f}, "
        ...
        if d['max_xg'] < 0.6:
            ...
            logger.warning("✗ Max xG > 0.95 — impossible in hockey, check for data leaks")
            all_passed = False
        else:
