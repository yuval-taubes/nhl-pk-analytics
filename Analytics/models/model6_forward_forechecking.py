"""Model 6: PK forward defensive-event profile.

Opponent-entry on-ice forechecking is unsupported without shift/tracking data.
This model ranks forwards by PK defensive events they directly participated in.
"""

import logging

import pandas as pd

from db import DatabaseConnection
from models.model_utils import add_timestamp, export_json
from models.player_scouting_utils import export_scouting, metric_ci

logger = logging.getLogger(__name__)
MODEL_NAME = "model6_forward_defensive_events"


class ForwardForecheckingModel:
    """Participant-based PK forward pressure/disruption profile."""

    def __init__(self, db_connection):
        self.db = db_connection
        self.data = None

    def fetch_data(self):
        query = """
        SELECT
            pl.player_id,
            pl.full_name,
            pl.position,
            g.season,
            e.game_id,
            e.event_id,
            e.event_type,
            CASE WHEN e.event_type = 'takeaway' THEN 1 ELSE 0 END AS takeaway,
            CASE WHEN e.event_type = 'blocked-shot' THEN 1 ELSE 0 END AS blocked_shot,
            CASE WHEN e.event_type = 'hit' THEN 1 ELSE 0 END AS hit,
            CASE WHEN e.event_type = 'giveaway' THEN 1 ELSE 0 END AS giveaway,
            CASE WHEN e.event_type = 'penalty' THEN 1 ELSE 0 END AS penalty
        FROM events e
        JOIN games g ON e.game_id = g.game_id
        JOIN event_players ep ON ep.event_id = e.event_id
        JOIN players pl ON pl.player_id = ep.player_id
        WHERE e.strength IN ('4v5', '3v5', '3v4', '5v4', '5v3', '4v3')
          AND ep.team_id = CASE
                WHEN split_part(e.strength, 'v', 1)::int < split_part(e.strength, 'v', 2)::int THEN g.away_team_id
                WHEN split_part(e.strength, 'v', 2)::int < split_part(e.strength, 'v', 1)::int THEN g.home_team_id
          END
          AND pl.position IN ('C', 'L', 'R', 'F', 'LW', 'RW')
          AND e.event_type IN ('takeaway', 'blocked-shot', 'hit', 'giveaway', 'penalty')
        """
        logger.info("Fetching Model 6 PK forward defensive events...")
        self.data = self.db.query_to_df(query)
        logger.info("Model 6 rows: %s", len(self.data))
        return self.data

    def summarize(self):
        rows = []
        for keys, group in self.data.groupby(["player_id", "full_name", "position"]):
            player_id, name, position = keys
            total = len(group)
            if total < 50:
                continue
            rows.append(
                {
                    "player_id": player_id,
                    "full_name": name,
                    "position": position,
                    "season": "ALL",
                    "defensive_events": total,
                    "takeaway_rate": float(group["takeaway"].mean()),
                    "block_rate": float(group["blocked_shot"].mean()),
                    "positive_event_rate": float((group["takeaway"] + group["blocked_shot"] + group["hit"]).clip(upper=1).mean()),
                    "negative_event_rate": float((group["giveaway"] + group["penalty"]).clip(upper=1).mean()),
                    "takeaway_rate_ci": metric_ci(group, lambda x: x["takeaway"].mean()),
                    "positive_event_rate_ci": metric_ci(group, lambda x: (x["takeaway"] + x["blocked_shot"] + x["hit"]).clip(upper=1).mean()),
                    "negative_event_rate_ci": metric_ci(group, lambda x: (x["giveaway"] + x["penalty"]).clip(upper=1).mean()),
                }
            )
        return pd.DataFrame(rows)

    def run(self):
        logger.info("=" * 60)
        logger.info("MODEL 6: PK Forward Defensive Event Profile")
        logger.info("=" * 60)
        self.fetch_data()
        summary = self.summarize()
        scouting_rows = []
        if not summary.empty:
            scouting_rows = export_scouting(
                self.db,
                MODEL_NAME,
                summary,
                "ALL",
                [
                    ("takeaway_rate", True, "defensive_events"),
                    ("positive_event_rate", True, "defensive_events"),
                    ("negative_event_rate", False, "defensive_events"),
                ],
            )
        results = add_timestamp(
            {
                "model": "PK Forward Defensive Event Profile",
                "players": summary.to_dict("records") if not summary.empty else [],
                "sample": {"raw_events": int(len(self.data)), "eligible_players": int(len(summary)), "minimum_events": 50},
                "caveats": [
                    "This replaces unsupported on-ice forechecking rankings with participant-event rankings",
                    "Counts only events where the forward is explicitly tagged in event_players",
                    "No per-60 rates are reported because true TOI is unavailable",
                    "This is descriptive, not causal attribution",
                ],
            }
        )
        results["scouting_rows_exported"] = len(scouting_rows)
        results["output_file"] = export_json(results, "model6_forward_defensive_events.json")
        logger.info("Model 6 output: %s", results["output_file"])
        return results


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(message)s")
    db = DatabaseConnection()
    db.connect()
    try:
        ForwardForecheckingModel(db).run()
    finally:
        db.close()
